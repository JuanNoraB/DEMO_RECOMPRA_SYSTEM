...
Qué hace:
  ...
  2. Usa por defecto todas las columnas numéricas del parquet.
  3. Entrena LightGBM.
  4. Extrae importancia por GAIN y SPLIT.
  5. Imprime únicamente las features que acumulan al menos 95% del gain.
  6. Guarda CSV en docs/feature_analysis/feature_summary.csv.
  7. Guarda log JSONL en data/logs/feature_importance_runs.jsonl (append).
  8. Guarda gráficos PNG en docs/feature_analysis/.
...
Ejemplo recomendado:
  python app/feature_importance.py --parquet data/features_store/features_train_full.parquet --compare-groups
...
def _plot_correlations(df: pd.DataFrame, feat_cols_base: list[str], out_dir: Path) -> pd.Series:
    ...
    try:
        ...
        sns.heatmap(corr_matrix, annot=True, fmt=".1f", cmap="RdBu_r", center=0, square=True, ax=ax, annot_kws={"size": font_size}, linewidths=0.3)
    except ImportError:
...
def main() -> None:
    ...
    n_neg = len(df) - n_pos
    print(f"[FeatImportance] {len(df):,} filas | {df['nucleo'].nunique():,} familias | target+={n_pos:,} ({df['target'].mean()*100:.1f}%) | target-={n_neg:,}")

    if args.use_config_features:
        feat_cols_base = [c for c in FEATURE_COLUMNS if c in df.columns]
        print(f"[FeatImportance] Modo FEATURE_COLUMNS: {len(feat_cols_base)} columnas base")
    else:
        feat_cols_base = [c for c in df.columns if c not in _SKIP_COLS and c != TIPO_CICLO_COL and df[c].dtype != object]
    ...
    if args.compare_groups:
        ...
        for _, row in compare_df.iterrows():
            print(f"{row['feature']:<35} {row['media_compradores']:>10.4f} {row['media_no_compra']:>10.4f} {row['diferencia_medias']:>12.4f}")
        print()
