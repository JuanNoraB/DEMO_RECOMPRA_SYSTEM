...
Qué hace:
  ...
  9. Guarda gráficos PNG en docs/feature_analysis/.
...
SPLIT: cuántas veces se usó la feature como punto de corte.
       → Útil como frecuencia, no como calidad.

Flags:
  --parquet PATH          parquet alternativo
  --use-config-features  usa FEATURE_COLUMNS en vez de todas las numéricas
  --compare-groups       imprime media/mediana por grupo

Ejemplo recomendado:
  python app/feature_importance.py --parquet data/features_store/features_train_all_1to4.parquet --compare-groups
...
Outputs:
  ...
  data/logs/feature_importance_runs.jsonl
"""
...
def _numeric_feature_columns(df: pd.DataFrame) -> list[str]:
    """Devuelve solo columnas numéricas reales. Excluye IDs, target y categóricas."""
    cols: list[str] = []
    skipped_non_numeric: list[str] = []

    for col in df.columns:
        if col in _SKIP_COLS or col == TIPO_CICLO_COL:
            continue
    ...
    if skipped_non_numeric:
        print(f"[FeatImportance] Columnas no numéricas excluidas: {skipped_non_numeric}")

    return cols


def _build_X(df: pd.DataFrame, feat_cols_base: list[str]) -> tuple[np.ndarray, list[str]]:
    ...
    for col in feat_cols_base:
        if not is_numeric_dtype(X_base[col]):
            raise TypeError(f"Columna no numérica detectada en X: {col} dtype={X_base[col].dtype}")
    ...
    X = np.nan_to_num(X_base.to_numpy(dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    return X, col_names


def _compare_groups(df: pd.DataFrame, feat_cols_base: list[str]) -> pd.DataFrame:
    """Compara distribución de features numéricas entre target=1 y target=0."""
    df1 = df[df["target"] == 1]
    df0 = df[df["target"] == 0]
    rows = []
...
def _plot_correlations(df: pd.DataFrame, feat_cols_base: list[str], out_dir: Path) -> pd.Series:
    """Genera corr_with_target.png y corr_features.png."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    numeric_cols = [c for c in feat_cols_base if c in df.columns and is_numeric_dtype(df[c])]
    if not numeric_cols:
        print("[FeatImportance] Sin columnas numéricas para correlaciones")
        return pd.Series(dtype=float)

    corr_target = df[numeric_cols + ["target"]].corr()["target"].drop("target").sort_values(ascending=False)
    ...
    for bar, val in zip(bars, corr_target.values[::-1]):
        ...
        ax.text(bar.get_width() + offset, bar.get_y() + bar.get_height() / 2, f"{val:.3f}", va="center", ha=ha, fontsize=8)
    plt.tight_layout()
    ...
    try:
        ...
        font_size = max(4, min(8, int(120 / max(n, 1))))
        sns.heatmap(
            corr_matrix,
            annot=True,
            fmt=".1f",
...
def select_until_threshold(result_df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    """Devuelve features hasta la primera fila donde acum_gain_pct >= threshold, inclusive."""
...
def main() -> None:
    ...
    parser.add_argument("--parquet", type=str, default=str(FEATURES_TRAIN_FILE), help="Path al parquet de features")
    parser.add_argument("--use-config-features", action="store_true", help="Usar FEATURE_COLUMNS en vez de todas las columnas numéricas")
    parser.add_argument("--compare-groups", action="store_true", help="Imprimir media/mediana por grupo")
    ...
    if "target" not in df.columns:
        raise ValueError("El parquet debe contener columna target")
    if "nucleo" not in df.columns:
        print("[FeatImportance] Aviso: no existe columna nucleo; se omite conteo de familias")

    n_pos = int(df["target"].sum())
    n_neg = len(df) - n_pos
    n_familias = int(df["nucleo"].nunique()) if "nucleo" in df.columns else -1
    print(
        f"[FeatImportance] {len(df):,} filas | {n_familias:,} familias | "
        f"target+={n_pos:,} ({df['target'].mean() * 100:.1f}%) | target-={n_neg:,}"
    )

    if args.use_config_features:
        feat_cols_base = [c for c in FEATURE_COLUMNS if c in df.columns and is_numeric_dtype(df[c])]
        print(f"[FeatImportance] Modo FEATURE_COLUMNS: {len(feat_cols_base)} columnas numéricas base")
    else:
        feat_cols_base = _numeric_feature_columns(df)
        print(f"[FeatImportance] Modo all-features por defecto: {len(feat_cols_base)} columnas numéricas base")

    if not feat_cols_base:
        raise ValueError("No hay columnas de features disponibles para entrenar")

    X, col_names = _build_X(df, feat_cols_base)
    print(f"[FeatImportance] Features enviadas al modelo: {len(col_names)}")
    ...
    top_95 = select_until_threshold(result_df, 95)
    print(f"\n[FeatImportance] Features que acumulan al menos 95% del gain ({len(top_95)} features):")
    print(f"{'#':>3}  {'Feature':<35} {'Gain%':>7} {'Acum%':>7} {'Split%':>7}")
    print("-" * 68)
    for _, row in top_95.iterrows():
        print(
            f"{int(row['rank']):>3}  {row['feature']:<35} "
            f"{row['gain_pct']:>7.2f} {row['acum_gain_pct']:>7.2f} {row['split_pct']:>7.2f}"
        )

    print(f"\n[FeatImportance] Total features evaluadas: {len(result_df)}")
    ...
    if args.compare_groups:
        ...
        if not compare_df.empty:
            ...
            for _, row in compare_df.iterrows():
                print(
                    f"{row['feature']:<35} {row['media_compradores']:>10.4f} "
    ...
    top_95_list = top_95[["rank", "feature", "gain_pct", "acum_gain_pct", "split_pct"]].to_dict(orient="records")
    run_log = {
        ...
        "modo_all_features": not args.use_config_features,
        "hparams_usados": "defaults",
        "n_estimators": n_estimators,
        "features_95pct": [r["feature"] for r in top_95_list],
        "features_95pct_detail": top_95_list,
    ...
    colors = ["#e74c3c" if g >= 5 else "#3498db" if g >= 1 else "#bdc3c7" for g in result_df["gain_pct"]]
