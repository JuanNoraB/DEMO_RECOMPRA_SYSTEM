...
- mediana_clientes: promedio de las medianas individuales.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Busqueda de horizontes globales de churn")
    p.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    p.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    p.add_argument("--alpha-min", type=float, default=1.0)
    p.add_argument("--alpha-max", type=float, default=4.0)
...
def main() -> None:
    ...
    B_media = float(perfil["media_cliente"].mean())
    B_mediana = float(perfil["mediana_cliente"].mean())

    bases = {
        "media_clientes": B_media,
        "mediana_clientes": B_mediana,
    }
    ...
    resultados = []
    for nombre_base, B in bases.items():
        for alfa in alphas:
            H = float(alfa * B)
    ...
    resumen = (
        ...
        f"clientes_con_intervalos: {len(perfil)}\n"
        f"B_global_media_clientes_dias: {B_media:.2f}\n"
        f"B_global_mediana_clientes_dias: {B_mediana:.2f}\n"
