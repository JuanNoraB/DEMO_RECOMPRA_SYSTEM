...
_ACT_MAP = {"relu": nn.ReLU, "tanh": nn.Tanh, "leaky_relu": nn.LeakyReLU}


class PurchaseFNN(nn.Module):
    def __init__(self, input_dim: int, hidden_dims: list = None, dropout: float = 0.3, activation: str = "relu"):
        super().__init__()
        ...
        for h in hidden_dims:
            layers += [nn.Linear(in_dim, h), nn.BatchNorm1d(h), act_fn(), nn.Dropout(dropout)]
            in_dim = h
...
def run_training(
    ...
    """
    features_path:      parquet de entrenamiento (features_train.parquet).
    eval_features_path: parquet de evaluación (features_eval.parquet).
                        Si es None evalúa sobre el mismo df de entrenamiento.
    save:               False durante HPT para no sobreescribir el modelo en cada trial.
    """
    ...
    # 90% para gradientes, 10% para early stopping (solo loss interno, no reportado)
    idx = np.arange(len(X))
    np.random.seed(42)
    np.random.shuffle(idx)
    s = int(len(X) * 0.9)
    X_tr = torch.tensor(X[idx[:s]])
    y_tr = torch.tensor(y[idx[:s]])
    X_es = torch.tensor(X[idx[s:]])
    y_es = torch.tensor(y[idx[s:]])

    # Peso para desbalance
    n_pos = y_tr.sum().item()
    pw = (len(y_tr) - n_pos) / max(n_pos, 1)

    _hidden = hidden_dims if hidden_dims is not None else [64, 32, 16]
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = PurchaseFNN(input_dim=len(feat_cols), hidden_dims=_hidden,
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.eval()
        with torch.no_grad():
            es_loss = criterion(model(X_es.to(device)), y_es.to(device)).item()
    ...
    if eval_features_path is not None:
        df_eval = pd.read_parquet(eval_features_path)
    else:
        df_eval = df
    ...
    topk_metrics = evaluate_topk(model, df_eval_aug, feat_cols, top_k=3)
...
def run_inference(features_path=None, output_path=None):
    ...
    model = PurchaseFNN(
        ...
        hidden_dims=meta.get("hidden_dims", [64, 32, 16]),
